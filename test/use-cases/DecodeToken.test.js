...
describe('decodeToken', function() {
  it('retuns false if the token does not have the jwt secret', async function() {
    const result = decodeToken('decoded_token', 'secret');

    expect(result).toBe(false);
  });

  it('retuns the decoded token if the encrypted token has the right signiture', async function() {
